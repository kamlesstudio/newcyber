    ...
    for pid in pids:
        try:
            ...
            with open(os.path.join('/proc', pid, 'cmdline'), 'rb') as cmd:
                ...
                if grep in output.decode('utf-8'):
                    return pid, cmd
...
def is_ips():
    ...
    suricata_output = psef(['suricata'])
#    print('suricata',suricata_output)
    if 'suricata' in suricata_output:
        return 'ok',200
    else:
        return 'error',404	
